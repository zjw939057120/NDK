...
private:
    ...
    //惯导模块加速度(0x5500+11字节数据帧)
    const uint8_t imu_buf_0x0551[CAN_BUFFER_LEN] = {0x55, 0x00, 0x55, 0x51, 0x4b, 0x00, 0x0a, 0x00, 0xff, 0x07, 0x8b,
                                                    0x0b, 0x9a};
    //惯导模块角速度(0x5500+11字节数据帧)
    const uint8_t imu_buf_0x0552[CAN_BUFFER_LEN] = {0x55, 0x00, 0x55, 0x52, 0xe5, 0xff, 0x3e, 0x00, 0x00, 0x00, 0x8b,
                                                    0x0b, 0x5f};
    //惯导模块加角度(0x5500+11字节数据帧)
    const uint8_t imu_buf_0x0553[CAN_BUFFER_LEN] = {0x55, 0x00, 0x55, 0x53, 0xf8, 0x00, 0x84, 0xfe, 0x6e, 0x0a, 0xfb,
                                                    0x46, 0xde};
    //惯导模块加磁场(0x5500+11字节数据帧)
    const uint8_t imu_buf_0x0554[CAN_BUFFER_LEN] = {0x55, 0x00, 0x55, 0x54, 0x83, 0x0c, 0x43, 0x26, 0x98, 0xe7, 0x00,
                                                    0x00, 0x20};
};
